...
import { MapPin, User, ChevronLeft, ChevronRight, ArrowRight, Loader2, Search, X, Tag, Package } from "lucide-react"
...
// junk (e.g. pasted code/log dumps) far longer than a real product name or
// description would ever be — this keeps the layout from ever breaking,
// independent of the CSS line-clamp used for normal truncation.
const HARD_CAP = {
    title: 120,
    tag: 40,
    description: 600,
...
export default function FeaturedProducts() {
    ...
    // A description only needs a "Read more" affordance once it's long enough
    // that it would otherwise get clipped by the 3-line clamp below.
    const DESC_PREVIEW_THRESHOLD = 140
        ...
        <section id="products" className="relative py-4 md:py-4 bg-[var(--background-secondary)] overflow-hidden px-4 sm:px-6 md:px-8">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div ref={searchBoxRef} className="relative max-w-md mx-auto mb-10 md:mb-14">
                    <div className="flex items-center border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 gap-2 shadow-[var(--shadow-sm)] rounded-sm focus-within:border-[var(--gold)] transition-colors">
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                        <ul className="absolute z-30 top-full left-0 right-0 bg-[var(--surface)] border border-[var(--border)] mt-1 max-h-60 overflow-y-auto shadow-[var(--shadow-lg)] rounded-sm">
                            {suggestions.map((s, i) => (
                                <li
                                    key={`${s.displayName}-${i}`}
                    ...
                    {showSuggestions && suggestions.length === 0 && searchedNoResults && !isSearchingLocations && (
                        <ul className="absolute z-30 top-full left-0 right-0 bg-[var(--surface)] border border-[var(--border)] mt-1 shadow-[var(--shadow-lg)] rounded-sm">
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                            {allProducts.map((product) => {
                                ...
                                // Defensively clamp text fields — some records can contain
                                // unexpectedly long/garbage text (e.g. pasted code), and
                                // this keeps every card the same height and shape.
                                const title = clamp(product.productName, HARD_CAP.title) || "Untitled product"
                                const category = clamp(product.category, HARD_CAP.tag)
                                const brand = clamp(product.brand, HARD_CAP.tag)
                                const description = clamp(product.description, HARD_CAP.description)
                                const isDescLong = description.length > DESC_PREVIEW_THRESHOLD
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className="group relative overflow-hidden border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-lg)] flex flex-col cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                                    >
                                        <div className="relative h-56 sm:h-64 w-full overflow-hidden shrink-0 bg-[var(--background-secondary)]">
                                        ...
                                        <div className="relative z-20 p-5 flex flex-col items-start flex-1">
                                            <h3
                                                className="text-lg sm:text-xl font-bold text-[var(--heading)] leading-snug mb-1 line-clamp-2 group-hover:text-[var(--gold)] transition-colors duration-300 font-[var(--font-heading)]"
                                                title={title}
                                            >
                                                {title}
                                            </h3>
                                            ...
                                            {brand && (
                                                <div className="flex items-center gap-1.5 mb-2">
                                                    <Tag size={11} className="text-[var(--muted)] shrink-0" />
                                                    <span className="text-xs text-[var(--muted)] font-medium truncate">{brand}</span>
                                                </div>
                                            )}
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-3">
                                                <div className="flex items-center gap-1.5 min-w-0">
                                                    ...
                                                    <span className="text-xs text-[var(--text)] font-light truncate max-w-[140px]">{supplier?.name || "Supplier"}</span>
                                                </div>

                                                {supplier?.city && (
                                                    <div className="flex items-center gap-1.5 min-w-0">
                                                        <MapPin size={12} className="text-[var(--muted)] shrink-0" />
                                                        <span className="text-xs text-[var(--text)] font-light truncate max-w-[140px]">
                                            ...
                                            {description && (
                                                <div className="mb-3 w-full">
                                                    <p className={`text-xs text-[var(--text)] opacity-80 leading-relaxed font-[var(--font-body)] ${isDescExpanded ? "" : "line-clamp-3"}`}>
                                                        {description}
                                                    </p>
                                                    {isDescLong && (
                                                        <button
                                                            onClick={(e) => toggleDescription(e, product.id)}
                                                            className="mt-1 text-[11px] font-semibold text-[var(--gold)] hover:text-[var(--heading)] transition-colors uppercase tracking-wide"
                                                        >
                                                ...
                                                {pricing.hasDiscount ? (
                                                    <>
                                                        <span className="text-xs text-[var(--text)] font-medium">
                                                            Original: ₹{pricing.original.toLocaleString("en-IN")}
                                                        </span>

                                                        <span className="flex items-center gap-1 text-[10px] text-[var(--gold)] font-semibold">
                                                            <Tag size={10} />
                                                            ₹{pricing.discount.toLocaleString("en-IN")} Off
                                                        </span>
                                                    </>
                                                ) : (
                                            ...
                                            <div className="flex items-center justify-between w-full border-t border-[var(--border)] pt-3 mt-3">
                                                <span className="text-[11px] text-[var(--muted)] font-[var(--font-body)] truncate max-w-[60%]">
                                                    {contact?.shopName || "View supplier"}
                                                </span>
                                                <span className="flex items-center gap-1.5 text-[11px] text-[var(--gold)] font-bold border-b border-[var(--gold)]/40 pb-0.5 group-hover:text-[var(--heading)] group-hover:border-[var(--heading)] transition-colors duration-300 shrink-0">
                                                    View Details
                                                    <ArrowRight size={12} />
                                                </span>
                                            </div>
